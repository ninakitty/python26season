...
def write_user_info(newuser_name, newuser_pwd):  # newuser_name,newuser_pwd 是形参接受实参
    '''
    将新用户的用户名,密码,初始购物卡金额写入文件
    :return:
    '''
    with open('用户信息.txt', 'a', encoding='utf-8') as f:
        f.write('%s \t%s \t%d\n' % (newuser_name, newuser_pwd, 0))  # 将刚注册的用户的名字和初始的购物卡余额（0）写入文件
        return
...
def recharge():
    if len(user_info_list) == 0:
        ...
        pay = int(input('请输入充值金额:').strip())
        money = int(user_info_list[2])
        money += pay
        user_info_list[2] = str(money)
        modify_money(user_info_list[0], str(money))
        print('充值成功')
